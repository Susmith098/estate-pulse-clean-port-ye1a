...
import { FiPlus, FiBarChart2, FiTrendingUp, FiHome, FiAlertCircle, FiGrid, FiChevronLeft, FiChevronRight, FiUsers } from 'react-icons/fi'
...
export default function BuilderView({ userId, inventory, buyerProfiles, onInventoryChange, setActiveAgentId }: BuilderViewProps) {
  ...
  // Reset page when inventory changes
  useEffect(() => { if (currentPage > totalPages) setCurrentPage(1) }, [items.length, totalPages, currentPage])
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-3 px-1">
                  <span className="text-xs text-slate-500">
                    Showing {(currentPage - 1) * pageSize + 1}-{Math.min(currentPage * pageSize, items.length)} of {items.length} units
                  </span>
