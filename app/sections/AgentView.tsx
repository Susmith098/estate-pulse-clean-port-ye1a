...
import { Users, Target, ChevronRight, RefreshCw, Copy, Check, AlertCircle, Loader2 } from 'lucide-react'
...
export default function AgentView({ userId, inventory, buyerProfiles, leads, onLeadSaved, setActiveAgentId }: AgentViewProps) {
  ...
  return (
    <div className="flex h-full gap-0 bg-[#0D0D24]">
      {/* Left Panel */}
      <div className="w-72 flex-shrink-0 border-r border-white/8 flex flex-col bg-[#0A0A1B]/50">
        <div className="px-4 py-3 border-b border-white/8">
          <h2 className="text-sm font-semibold text-white flex items-center gap-2">
            ...
            Buyer Profiles
            <Badge className="bg-white/8 text-slate-400 border-white/10 border text-xs ml-auto">{allProfiles.length}</Badge>
          </h2>
        </div>
          ...
          <div className="p-2 space-y-1">
            {allProfiles.length === 0 && (
              <div className="text-center py-10 px-4">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">No buyer profiles yet. They'll appear after buyers complete their preferences.</p>
              </div>
            )}
            {allProfiles.map((profile: any) => {
              const pLead = allLeads.find((l: any) => l?.buyer_profile_id === profile?._id) ?? leadData[profile?._id ?? '']
              const scoreStyle = pLead ? getScoreStyle(pLead.lead_score) : null
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {scoreStyle && pLead && (
                      <Badge className={`text-xs border ${scoreStyle.badge}`}>{pLead?.lead_score}</Badge>
                    )}
                  </div>
      ...
      <div className="flex-1 overflow-hidden">
        {!selectedProfileId ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            ...
            <p className="text-sm text-slate-400 max-w-sm leading-relaxed">Select a buyer profile from the left panel to generate AI-powered lead insights, scores, and personalized pitch messages.</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    >
                      {loading === selectedProfileId
                        ...
                        : <><RefreshCw className="w-3.5 h-3.5 mr-1.5" /> Generate Insights</>}
                    </Button>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 text-xs">
                    {[
                      ...
                      <div key={item.label} className="p-2.5 rounded-xl bg-white/5 border border-white/8">
                        ...
                        <span className="font-medium text-slate-200 mt-0.5 block">{item.value}</span>
                      </div>
                    ))}
                  </div>
                </CardContent>
                  ...
                  <Card className="bg-white/5 border border-white/10 rounded-2xl">
                    <CardContent className="p-4">
                      <div className="flex items-center gap-3 mb-3">
                        <Badge className={`text-base px-4 py-1 border ${getScoreStyle(currentLead.lead_score).badge}`}>
                          ...
                          {currentLead.recommended_units.map((unit: any, i: number) => (
                            <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/8">
                              <div>
                                ...
                                <span className="text-xs ml-2 text-slate-500">{unit?.unit_number ?? ''}{unit?.bhk ? ` · ${unit.bhk}BHK` : ''}{unit?.area_sqft ? ` · ${unit.area_sqft}sqft` : ''}</span>
                              </div>
                    ...
                    <Card className="bg-white/5 border border-white/10 rounded-2xl">
                      <CardHeader className="pb-2">
                        <div className="flex items-center justify-between">
                          ...
                          >
                            {copiedField === 'pitch' ? <><Check className="w-3 h-3 mr-1 text-emerald-400" /> Copied</> : <><Copy className="w-3 h-3 mr-1" /> Copy</>}
                          </Button>
                        </div>
                      </CardHeader>
                      <CardContent className="pt-0 px-4 pb-4">
